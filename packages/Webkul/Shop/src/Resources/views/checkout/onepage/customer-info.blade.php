<form data-vv-scope="address-form">
    <div v-if="!this.new_billing_address">
        <div class="deliver-sect" v-for='(addresses, index) in this.allAddress'>
            ...
            <!-- <div class="radio2"> 
                ...
                <input type="radio" v-validate="'required'" id="billing[address_id]" name="billing[address_id]" :value="addresses.id" v-model="address.billing.address_id" data-vv-as="&quot;{{ __('shop::app.checkout.onepage.billing-address') }}&quot;">
                <label for="radio-1" class="radio-label">@{{ allAddress.first_name }} @{{ allAddress.last_name }}</label>
                <span class="checkmark"></span>
                <input type="hidden" value=index>
            </div> -->
          
            <div class="top-t">
               <p class="address2">@{{ addresses.company_name }}, @{{ addresses.address1 }}, @{{ addresses.city }}, @{{ addresses.state }}<br> @{{ addresses.country }} - @{{ addresses.postcode }}</p>
               <p class="mob-no">Mobile:  <strong>@{{ addresses.phone }}</strong></p>
            </div> 
                
             <!--<div class="btn-right"><button class="edit-btn" type="button">Edit</button>
        ...
        <div class="add-address">
            <a  @click = newBillingAddress()>
                <span class="add-new"><img src="{{ asset('/themes/velocity/assets/images/fusion-door-payment/add-new.png') }}" alt=""></span>
                  <p>  {{ __('shop::app.checkout.onepage.new-address') }}</p>
            </a>
        </div>
    </div>
    ...
    <div class="form-container" v-if="this.new_billing_address">
        ...
        <div class="control-group" :class="[errors.has('address-form.billing[company_name]') ? 'has-error' : '']" style="display:none;">
