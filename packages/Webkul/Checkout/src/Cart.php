    ...
    public function addProduct($productId, $data)
    {
        ...
        if (is_string($cartProducts)) {
            ...
            foreach ($cartProducts as $cartProduct) {
                ...
                if(isset($data['priceradio']))
                {
                    
                    if($data['priceradio'] == '100')
                    {
                        $finalAmount = $data['actual_amount'];

                    }
                    else
                    {
                        $finalAmount = $data['percent_amount'];
                    }
                }

                if(isset($cartProduct['price']))
                {
                    $cartProduct['actual_price'] = $cartProduct['price'];
                    $cartProduct['custom_price'] = $finalAmount;
                }
        ...
        {            
            ...
            {                
                ...
                {    
                    ...
                    {
                        $attributeIdArray = explode('_',$attributeId);
                        if(isset($attributeIdArray[0]))
                        {
                            $attr = $attributeIdArray[0];
                        }
                        if($attributeIdArray[1] == $index)
                        {
                            DB::table('cart_items')
                            ->where('id', $index)
                            ->update(['additional->super_attribute->'.$attr => $optionId]);        
                        }
                    }         
                    // echo"<pre>";print_r($data);die; 
                }
